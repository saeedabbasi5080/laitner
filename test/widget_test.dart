...
void main() {
  test('ReviewCardUseCase applies know/dontKnow rules', () {
    final useCase = ReviewCardUseCase(_FakeFlashcardRepository());
    ...
    expect(
      useCase.applyReview(card, ReviewRating.dontKnow, now: now).box,
      1,
    );
    expect(
      useCase.applyReview(card, ReviewRating.know, now: now).box,
      3,
    );
    expect(
      useCase
